...
//! verbs) and a common ancestor of both consumers — `spark-model`'s SSM tier
//! fingerprint and `spark-storage`'s KV paging namespace. Previously each had
//! its own copy, which is exactly the SSOT violation the repo conventions warn
//! against: three transcriptions of the same magic constants, free to drift.
//!
