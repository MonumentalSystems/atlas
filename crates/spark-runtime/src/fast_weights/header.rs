...
pub(super) fn parse_header(file: &mut File) -> Result<Vec<TensorMeta>> {
    ...
    for (name, info) in obj {
        ...
        // Shared closed mapping (see `WeightDtype::from_safetensors_str`). The
        // RDMA weight peer publishes the same raw dtype strings, so both the
        // disk and RDMA loaders resolve dtypes identically. I8 → UInt8 (raw
        // 4-bit-packed NVFP4 container; signedness irrelevant).
