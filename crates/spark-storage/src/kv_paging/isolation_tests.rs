...
use crate::snapshot_swap::{MemSwapStore, SnapshotResidency, VecSlotArena};
...
type MockPeer = SnapshotResidency<VecSlotArena, MemSwapStore>;
...
fn peer(slots: usize) -> MockPeer {
    SnapshotResidency::new(VecSlotArena::new(BB, slots), MemSwapStore::new(BB)).unwrap()
}
...
// coincide. Pin that structural guarantee (Step A must never collapse it) —
