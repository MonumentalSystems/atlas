...
pub(super) fn generate_target_ptx_rs(
    ...
    // Metadata and module registry for every compiled backend. Kernel blobs
    // are backend-neutral bytes, so Metal uses the same target lookup surface
    // while loading them through `metallib_modules()`.
    g.push_str(
