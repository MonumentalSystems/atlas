...
impl TransformerModel {
    ...
    pub(super) fn upload_batch_metadata_fixed(
        ...
        // init). Fixed address, per-step contents → graph-safe + route-agnostic.
        // `DevicePtr(0)` when no adapter is resident (the MoE fold hooks then
        // take the request-granularity gate). Distinct from `seq_slot` (MoE
        // `-1 = base`).
        let moe_row_adapter =
    ...
    pub(super) fn upload_batch_metadata_at(
        ...
        // moe_row_adapter_buf (fixed address).
        let moe_row_adapter =
            self.upload_moe_row_adapter(seqs, padded_n, self.moe_row_adapter_buf, stream)?;
