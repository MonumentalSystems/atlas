...
//! `ATLAS_HOLO_MOE_B12X=1`. Mirrors `gdn_flashinfer.rs` exactly (raw `dlopen`/`dlsym`,
...
fn lib() -> Option<&'static Lib> {
    LIB.get_or_init(|| unsafe {
        ...
        if h.is_null() {
            tracing::warn!(
                "ATLAS_HOLO_MOE_B12X: dlopen('{path}') failed — falling back to grouped"
            );
            return None;
        }
        ...
        if load.is_null() || prefill.is_null() || max_tokens.is_null() {
            tracing::warn!(
                "ATLAS_HOLO_MOE_B12X: symbols not found in lib — falling back to grouped"
            );
            return None;
        }
        ...
        if cap <= 0 {
            tracing::warn!(
                "ATLAS_HOLO_MOE_B12X: atlas_b12x_max_tokens()={cap} (export absent?) — grouped"
            );
            return None;
        }
        tracing::info!(
            "ATLAS_HOLO_MOE_B12X: FlashInfer b12x fused-MoE loaded (opt-in, max_tokens={cap}); \
...
/// True when `ATLAS_HOLO_MOE_B12X=1` AND the library + symbols loaded successfully.
pub(crate) fn available() -> bool {
    std::env::var("ATLAS_HOLO_MOE_B12X").as_deref() == Ok("1") && lib().is_some()
}
