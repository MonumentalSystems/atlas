...
/// b12x fused-MoE repacked weights (`ATLAS_MOE_B12X`). Device buffers are process-
...
pub(crate) enum B12xEligibility {
    ...
    Build,
    /// `--stream-experts` + `ATLAS_MOE_B12X=1` — user misconfiguration, hard error.
    ErrStreamer,
...
impl MoeLayer {
    ...
    /// Build b12x fused-MoE weights at load (behind `ATLAS_MOE_B12X`). Sets
    ...
    pub(crate) fn build_b12x_weights(
        ...
        if config.num_experts != 256
            || config.hidden_size != 2048
            || config.moe_intermediate_size != 512
            || config.num_experts_per_tok != 8
        {
            tracing::warn!(
                "ATLAS_MOE_B12X: unsupported geometry E={} H={} I={} top_k={} — disabled",
                config.num_experts,
                config.hidden_size,
        ...
        // No streaming-experts stack on this branch — b12x is resident-only here, so a
        // streamer can never be attached. The `eligibility` gate keeps the `has_streamer`
        // arm (and its ErrStreamer bail) so the check stays correct if streaming lands later.
        ...
        match eligibility(has_streamer, config.ep_world_size, any_null, have_t) {
            B12xEligibility::ErrStreamer => anyhow::bail!(
                "ATLAS_MOE_B12X=1 is incompatible with --stream-experts: b12x enforces \
                 num_local_experts == num_experts (fully-resident experts only). Unset one."
            ),
            B12xEligibility::SkipEp => {
                tracing::warn!(
                    "ATLAS_MOE_B12X: ep_world_size={} > 1 — b12x disabled, grouped path runs",
            ...
            B12xEligibility::SkipNullExpert => {
                tracing::warn!(
                    "ATLAS_MOE_B12X: null/placeholder expert(s) present — b12x disabled, grouped"
                );
                return Ok(());
            }
            B12xEligibility::SkipNoTables => {
                tracing::warn!(
                    "ATLAS_MOE_B12X: transposed _t tables absent (need FAST_MOE=full) — grouped"
                );
                return Ok(());
            }
            B12xEligibility::Build => {}
        ...
        let max_tokens = match ops::b12x_flashinfer::max_tokens() {
            ...
            None => {
                tracing::warn!(
                    "ATLAS_MOE_B12X: libatlasb12x.so not loaded (max_tokens unavailable) — \
        ...
        tracing::info!(
            "ATLAS_MOE_B12X: built fused weights for {e_count} experts (H={h} I={inter}, \
