        ...
        while s < n {
            ...
            if k >= 2 {
                ...
                for j in 0..k {
                    ...
                    fwd.gpu
                        .copy_d2d_async(k_scratch.offset(j * kv_bytes), k_out, kv_bytes, stream)?;
                    fwd.gpu
                        .copy_d2d_async(v_scratch.offset(j * kv_bytes), v_out, kv_bytes, stream)?;
                    if !self.attn.q_norm.weight.is_null() {
                        ops::rms_norm(
                            fwd.gpu,
                            self.rms_norm_k,
                ...
                let k_out = q_out.offset(q_proj_bytes);
                let v_out = k_out.offset(kv_bytes);
                self.ms_qkv_seq_q(fwd, normed_base, q_out, q_proj_dim, q_dim, nq, hd, h, stream)?;
