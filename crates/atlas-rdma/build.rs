...
// DIRECT dependents as `DEP_ATLAS_RDMA_SHIM_HAS_VERBS`; spark-storage's
// build.rs reads it and re-emits `cargo:rustc-cfg=atlas_rdma_verbs` for its
// own nine gated modules. Any future crate that grows a real
// `#[cfg(atlas_rdma_verbs)]` gate needs its own DIRECT dep on atlas-rdma plus
// the same re-emit (DEP_ vars are invisible to transitive dependents).
