...
// peer daemons (moved verbatim from spark-storage's `expert_peer.rs` /
// `cache_peer.rs` in the RailSet extraction, Step B; re-exported there at the
// old paths so the servers are zero-diff).
//
// ** GOLDEN WIRE FORMAT ** — the live gx10 peer runs an OLDER binary, so
// every byte layout here is frozen. All integers little-endian. Exact byte
// vectors are pinned by `spark-storage/tests/rdma_wire_golden.rs` (written
// against the pre-move codecs) and `tests/wire_roundtrip.rs` here.
//
