...
fn peer_hangup_before_rail_params_names_the_rejection_not_a_buffer_error() {
    ...
    let err = read_rw_server_params(&mut s, 1, "gx10:9931").unwrap_err();
    let msg = format!("{err:#}");
...
fn a_truncated_params_body_is_still_a_read_error_not_a_rejection() {
    ...
    assert!(msg.contains("params"), "should blame the params read: {msg}");
}
...
fn a_rail_count_mismatch_is_reported_as_a_grant_mismatch() {
    ...
    let mut s = TruncatedPeer { remaining: vec![3u8] };
    let err = read_rw_server_params(&mut s, 1, "gx10:9931").unwrap_err();
    let msg = format!("{err:#}");
