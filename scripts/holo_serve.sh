...
setsid -f env RUST_BACKTRACE=1 RUST_LOG=info \
  ...
  "$BIN" serve \
    ...
    --max-prefill-tokens 4096 --kv-cache-dtype bf16 --lm-head-dtype bf16 \
    --gpu-memory-utilization 0.52 --oom-guard-mb 256 --ssm-cache-slots 0 --ssm-checkpoint-interval 0 \
