...
def _resolve_str_annotations(fn):
    ...
    resolve = {
        ...
        "cuda.CUstream": _cuda_aot.CUstream,
    }
...
def main():
    ...
    ap.add_argument(
        "--static-m",
        default="",
        help="comma-separated decode batch sizes to AOT-export (for example: 4,8)",
    )
    args = ap.parse_args()
    ...
    static_ms = [int(value) for value in args.static_m.split(",") if value.strip()]
    for m in static_ms:
        if m <= 0:
            raise SystemExit(f"--static-m values must be positive, got {m}")
        _captured.clear()
        _, mac = _get_static_kernel(
            state_E=E,
            weight_E=E,
            m=m,
            k=H,
        ...
        if not _captured:
            raise SystemExit(f"static m={m} compile produced no exportable executor")
        static_name = f"b12x_static_m{m}"
        static_cf = _captured[-1]
        static_cf.export_to_c(args.out, static_name, static_name)
        print(f"EXPORTED static m={m} mac={mac} -> {args.out}/{static_name}.{{h,o}}")
