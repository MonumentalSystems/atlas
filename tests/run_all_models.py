...
class TestSpec:
    ...
    vision: bool = False
    # Mark DFlash speculative-decode rounds so the suite runs its acceptance
    # check (`--dflash-check`). The `--dflash`/`--draft-model`/`--dflash-gamma`
    # serve flags are passed through `extra_args`. The drafter's `rope_scaling`
    # must match the engine's RoPE assumption — the published Qwen3.5-A3B-DFlash
    # drafter is `rope_scaling=null` (standard RoPE); the Qwen3.6 drafter is YaRN.
    dflash: bool = False
...
ROUNDS: List[List[tuple]] = [
    ...
    # Round 12: Holo-3.1-35B-A3B + DFlash speculative decode (head). Pairs the
    # Qwen3.5-A3B target with its production drafter z-lab/Qwen3.5-35B-A3B-DFlash
    # (rope_scaling=null → standard RoPE, which the engine assumes). Runs the
    # acceptance check (`--dflash-check`): the suite asserts the K=2 verify
    # accept-rate is above the drift floor and reports tok/s vs the non-spec
    # baseline. Requires the drafter checkpoint cached on the head node.
    [
        ("head", TestSpec(
            "holo-3.1-35B-dflash", "Hcompany/Holo-3.1-35B-A3B-NVFP4",
            dflash=True,
            extra_args=[
                "--dflash",
